...
static void * switch_sai_thrift_rpc_server_thread(void *arg) {
  int port = *(int *) arg;
  std::cerr << "switch_sai_thrift_rpc_server_thread " << std::endl;
  shared_ptr<switch_sai_rpcHandler> handler(new switch_sai_rpcHandler());
  std::cerr << "switch_sai_thrift_rpc_server_thread new switch_sai_rpcHandler" << std::endl;
  shared_ptr<TProcessor> processor(new switch_sai_rpcProcessor(handler));
  std::cerr << "switch_sai_thrift_rpc_server_thread new switch_sai_rpcProcessor" << std::endl;
  shared_ptr<TServerTransport> serverTransport(new TServerSocket(port));
  std::cerr << "switch_sai_thrift_rpc_server_thread new TServerSocket" << std::endl;
  shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
  std::cerr << "switch_sai_thrift_rpc_server_thread new TBufferedTransportFactory" << std::endl;
  shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
  std::cerr << "switch_sai_thrift_rpc_server_thread new TBinaryProtocolFactory" << std::endl;
...
int start_p4_sai_thrift_rpc_server(int port)
{
    ...
    *port_arg = port;
    ...
    rc = pthread_create(&switch_sai_thrift_rpc_thread, NULL, switch_sai_thrift_rpc_server_thread, port_arg);
